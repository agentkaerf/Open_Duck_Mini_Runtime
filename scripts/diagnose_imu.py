...
def test_standing(imu, args):
    ...
    if overall == "PASS":
        ...
        print(f"  Residual pitch error to correct: {signed_pitch:+.2f} deg")
        print()
        print("  NO WORKING CORRECTION KNOB EXISTS FOR THIS PATH YET:")
        print("   * --pitch_bias is currently a NO-OP here. The walk runtime uses")
        print("     the raw gyro/accel backend, which accepts user_pitch_bias and")
        print("     never applies it (only imu.py, the quaternion backend behind")
        print("     imu_server.py, honours it). Setting it will change nothing.")
        print("   * Do NOT use tare_x(): it forces accel X to zero, which deletes")
        print("     the legitimate -0.372 gravity component of the home pose and")
        print("     manufactures a ~2.2 deg lean in the opposite direction.")
        print()
        print("  First confirm the axis remap is right (--test axes): a remap error")
        print("  masquerades as a bias, and fixing it may remove this entirely.")
        print("  If a real bias survives a correct remap, pitch correction has to")
        print("  be implemented in the raw backend before it can be applied.")
